# Databricks – Sports Bar & Atliqon Data Integration

This project builds an end‑to‑end analytics pipeline in Databricks to integrate Sports Bar data stored in AWS S3 with existing, already‑cleaned Atliqon data stored in a Gold table.
...
![Project Architecture](https://github.com/ShivamMahto2105/End-to-End-ETL-Pipeline-for-Enterprise-Acquisition-and-Unified-Analytics/blob/main/Resource/project_architecture.png)

*High-level view of the Bronze → Silver → Gold medallion pipeline, from Sports Bar (S3) and Atliqon sources through to the dashboard.*

## Project Goals

- **Integrate datasets:** Merge Sports Bar data from S3 with Atliqon data already modelled in a Gold table.
...
Codes/ – All Databricks notebooks for setup and transformations.
...
2. **Core Tables & Dimensions**
   ...
   - These will read from `data/child_company_data` and/or S3 Sports Bar data, transform them through Bronze/Silver, and publish gold‑layer dimensions.
...
## Notes 
Go through the following _______ article for more information.